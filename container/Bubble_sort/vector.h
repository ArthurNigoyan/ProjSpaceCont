...
class vectorclass
{
	...
	bool empty();
	vectorclass<T>& operator[](int index);
	// bool operator>(vectorclass<T> a1, vectorclass<T> a2);
...
template <typename T>
vectorclass<T>& vectorclass<T>::operator[](int index)
{
    if (index >= m_size) {
...
// template <typename T>
// bool vectorclass<T>::operator>(vectorclass<T> a1, vectorclass<T> a2)
// {
    // return a1>a2;
// }
#endif
